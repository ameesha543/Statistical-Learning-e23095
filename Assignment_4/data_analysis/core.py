...
class DataInspector:
    ...
    Demonstrated using the UCI Automobile Dataset (car price prediction).
    ...
    def get_categorical_summary(self):
        ...
        including unique counts, mode value, and mode frequency.
        """
        if self.df is None: return
    ...
    def plot_all_associations_heatmap(self):
        """
        Creates a unified association matrix for ALL column type pairs and
        displays it as a single interactive Plotly Heatmap.
    ...
    def test_constant_covariance(self, columns: Optional[Sequence[str]] = None, chunks: int = 5) -> Any:
        ...
        chunk_size = n // chunks
        if chunk_size <= m:
            raise ValueError(f"Degrees of freedom per chunk must exceed dimensions.")
    ...
    def test_row_independence(self, columns: Optional[Sequence[str]] = None, max_lag: Optional[int] = None) -> Any:
        ...
        if max_lag >= n: raise ValueError(f"Max lag must be less than sample size.")
    ...
    def compute_empirical_pca(self, columns: Optional[Sequence[str]] = None, show_plot: bool = True) -> Dict[str, Any]:
        ...
        Computes Hotelling's T^2 and Q (SPE) across all truncation boundaries k.
        Generates a 2x3 Plotly Dashboard.
        """
        if self.df is None: raise ValueError("Error: No data loaded.")
        if columns is None:
            target_cols = self.df.select_dtypes(include=[np.number]).columns.tolist()
        ...
        if show_plot:
            ...
            fig.add_trace(go.Bar(x=pc_labels, y=lambda_hat, name="Eigenvalue (λ)",
        ...
        return {"mean_vector": mu_hat, "covariance_matrix_S": S_matrix,
                ...
                "transformed_scores_Z": Z_scores, "features": target_cols,
    ...
    def compute_empirical_fa(self, k: int, columns: Optional[Sequence[str]] = None, show_plot: bool = True) -> Dict[str, Any]:
        """
        Decomposes the empirical correlation matrix R into shared structural variance
        (Lambda * Lambda^T) and uniqueness (Psi) via Factor Analysis.
        ...
        print(f"\n--- FA | {m} features → {k} latent factors ---")
        ...
        return {"mean_vector_mu": mu_hat, "correlation_matrix_R": R_matrix,
                "factor_loadings_lambda": lambda_matrix, "uniqueness_psi": uniqueness,
                "communality_h2": communality, "latent_factor_scores_F": F_scores,
                "sensors": target_cols}
